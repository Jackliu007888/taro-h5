...
declare namespace TaroH5 {
   ...
   *
   * 微信端能力
   * original code from: https://github.com/qiu8310/minapp/blob/master/packages/minapp-wx/typing/TaroH5.d.ts
   ...
   *
   *     ```javascript
   *     const requestTask = TaroH5.request({
   *       url: 'test.php', //仅为示例，并非真实的接口地址
   *       data: {
   *          x: '' ,
   *          y: ''
   ...
   *
   * 如页面通过 [TaroH5.chooseImage](https://developers.weixin.qq.com/miniprogram/dev/api/media-picture.html#wxchooseimageobject) 等接口获取到一个本地资源的临时文件路径后，可通过此接口将本地资源上传到指定服务器。
   *
   * **返回值：**
   *
   * @since 1.4.0
   ...
   *
   *     ```javascript
   *     const uploadTask = TaroH5.uploadFile({
   ...
   *
   *     ```javascript
   *     const downloadTask = TaroH5.downloadFile({
   ...
   *
   * **基础库 1.7.0 之前，一个微信小程序同时只能有一个 WebSocket 连接，如果当前已存在一个 WebSocket 连接，会自动关闭该连接，并重新创建一个 WebSocket 连接。基础库版本 1.7.0 及以后，支持存在多个 WebSokcet 连接，每次成功调用 TaroH5.connectSocket 会返回一个新的 [SocketTask](https://developers.weixin.qq.com/miniprogram/dev/api/socket-task.html)。**
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.connectSocket({
   ...
   *       url: 'test.php'
   *     })
   *     TaroH5.onSocketOpen(function(res) {
   ...
   *       console.log('WebSocket连接已打开！')
   *     })
   *     TaroH5.onSocketError(function(res){
  ...
  /**
   * 通过 WebSocket 连接发送数据，需要先 [TaroH5.connectSocket](https://developers.weixin.qq.com/miniprogram/dev/api/network-socket.html#wxconnectsocketobject)，并在 [TaroH5.onSocketOpen](https://developers.weixin.qq.com/miniprogram/dev/api/network-socket.html#wxonsocketopencallback) 回调之后才能发送。
   *
   * **示例代码：**
   *
   *     ```javascript
   ...
   *     })
   *
   *     TaroH5.onSocketMessage(function(res) {
   ...
   *
   *     //注意这里有时序问题，
   *     //如果 TaroH5.connectSocket 还没回调 TaroH5.onSocketOpen，而先调用 TaroH5.closeSocket，那么就做不到关闭 WebSocket 的目的。
   *     //必须在 WebSocket 打开期间调用 TaroH5.closeSocket 才能关闭。
   *     TaroH5.onSocketOpen(function() {
   *       TaroH5.closeSocket()
   *     })
   *
   *     TaroH5.onSocketClose(function(res) {
   ...
   * @since 1.7.0
   *
   * WebSocket 任务，可通过 [TaroH5.connectSocket()](https://developers.weixin.qq.com/miniprogram/dev/api/network-socket.html) 接口创建返回。
   */
  class SocketTask {
    /**
     *
  ...
  /**
   * **注意：1.6.0 版本开始，本接口不再维护。建议使用能力更强的 [TaroH5.getRecorderManager](https://developers.weixin.qq.com/miniprogram/dev/api/getRecorderManager.html) 接口**
   *
   * 开始录音。当主动调用`TaroH5.stopRecord`，或者录音超过1分钟时自动结束录音，返回录音文件的临时文件路径。当用户离开小程序时，此接口无法调用。
   *
   ...
   *
   *     ```javascript
   *     const recorderManager = TaroH5.getRecorderManager()
   *
  ...
  /**
   * **注意：1.6.0 版本开始，本接口不再维护。建议使用能力更强的 [TaroH5.createInnerAudioContext](https://developers.weixin.qq.com/miniprogram/dev/api/createInnerAudioContext.html) 接口**
   *
  ...
  /**
   * 暂停正在播放的语音。再次调用TaroH5.playVoice播放同一个文件时，会从暂停处开始播放。如果想从头开始播放，需要先调用 TaroH5.stopVoice。
   *
   * **示例代码：**
   *
   *     ```javascript
  ...
  /**
   * **注意：1.2.0 版本开始，本接口不再维护。建议使用能力更强的 [TaroH5.getBackgroundAudioManager](https://developers.weixin.qq.com/miniprogram/dev/api/getBackgroundAudioManager.html) 接口**
   *
   * 获取后台音乐播放状态。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.getBackgroundAudioPlayerState({
   *         success: function(res) {
   ...
   * **bug & tip：**
   *
   * 1.  `bug`: `iOS` `6.3.30` TaroH5.seekBackgroundAudio 会有短暂延迟
   ...
   *
   *     ```javascript
   *     const backgroundAudioManager = TaroH5.getBackgroundAudioManager()
   *
   ...
   *     Page({
   *       onReady: function (e) {
   *         // 使用 TaroH5.createAudioContext 获取 audio 上下文 context
   *         this.audioCtx = TaroH5.createAudioContext('myAudio')
   ...
   * @since 1.6.0
   *
   * 创建并返回内部 audio 上下文 `innerAudioContext` 对象。_本接口是 `TaroH5.createAudioContext` 升级版。_
   *
   * **errCode 说明：**
   *
   *   errCode   |  说明
   ...
   *
   *     ```javascript
   *     const innerAudioContext = TaroH5.createInnerAudioContext()
   ...
   *         this.videoContext = TaroH5.createVideoContext('myVideo')
   *       },
   *       inputValue: '',
  ...
  /**
   * 获取本地文件的文件信息。此接口只能用于获取已保存到本地的文件，若需要获取临时文件信息，请使用 [TaroH5.getFileInfo](https://developers.weixin.qq.com/miniprogram/dev/api/getFileInfo.html) 接口。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.getSavedFileInfo({
   ...
   *     ```javascript
   *     try {
   *         TaroH5.setStorageSync('key', 'value')
   *     } catch (e) {
   *     }
   *     ```
   ...
   *     ```javascript
   *     try {
   *       var value = TaroH5.getStorageSync('key')
   *       if (value) {
   ...
   *     ```javascript
   *     try {
   *       var res = TaroH5.getStorageInfoSync()
   *       console.log(res.keys)
   *       console.log(res.currentSize)
   *       console.log(res.limitSize)
   *     } catch (e) {
  ...
  namespace getLocation {
    ...
    type Param = {
      /**
       * 默认为 wgs84 返回 gps 坐标，gcj02 返回可用于`TaroH5.openLocation`的坐标
       */
      type?: string
      /**
   ...
   *     ```javascript
   *     TaroH5.getLocation({
   *       type: 'gcj02', //返回可以用于TaroH5.openLocation的经纬度
   *       success: function(res) {
   *         var latitude = res.latitude
   *         var longitude = res.longitude
   *         TaroH5.openLocation({
   ...
   *     Page({
   *       onReady: function (e) {
   *         // 使用 TaroH5.createMapContext 获取 map 上下文
   *         this.mapCtx = TaroH5.createMapContext('myMap')
   *       },
  ...
  class MapContext {
    /**
     * 获取当前地图中心的经纬度，返回的是 gcj02 坐标系，可以用于 [`TaroH5.openLocation`](https://developers.weixin.qq.com/miniprogram/dev/api/location.html#wxopenlocationobject)
     */
   ...
   *     ```javascript
   *     try {
   *       var res = TaroH5.getSystemInfoSync()
   *       console.log(res.model)
   *       console.log(res.pixelRatio)
   *       console.log(res.windowWidth)
   *       console.log(res.windowHeight)
   ...
   *
   *     ```js
   *     TaroH5.canIUse('openBluetoothAdapter')
   *     TaroH5.canIUse('getSystemInfoSync.return.screenWidth')
   *     TaroH5.canIUse('getSystemInfo.success.screenWidth')
   *     TaroH5.canIUse('showToast.object.image')
   *     TaroH5.canIUse('onCompassChange.callback.direction')
   *     TaroH5.canIUse('request.object.method.GET')
   *
   *     TaroH5.canIUse('live-player')
   *     TaroH5.canIUse('text.selectable')
   *     TaroH5.canIUse('button.open-type.contact')
   *     ```
   ...
   *
   *     ```javascript
   *     TaroH5.onNetworkStatusChange(function(res) {
  ...
  /**
   * 监听加速度数据，频率：5次/秒，接口调用后会自动开始监听，可使用 `TaroH5.stopAccelerometer` 停止监听。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.onAccelerometerChange(function(res) {
   *       console.log(res.x)
   *       console.log(res.y)
   *       console.log(res.z)
   *     })
  ...
  /**
   * 监听罗盘数据，频率：5次/秒，接口调用后会自动开始监听，可使用`TaroH5.stopCompass`停止监听。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.onCompassChange(function (res) {
   ...
   *
   *     ```javascript
   *     TaroH5.setClipboardData({
   *       data: 'data',
   *       success: function(res) {
   *         TaroH5.getClipboardData({
   *           success: function(res) {
   ...
   * @since 1.1.0
   *
   * 初始化小程序蓝牙模块，生效周期为调用`TaroH5.openBluetoothAdapter`至调用`TaroH5.closeBluetoothAdapter`或小程序被销毁为止。 在小程序蓝牙适配器模块生效期间，开发者可以正常调用下面的小程序API，并会收到蓝牙模块相关的on回调。
   *
   * **Bug & Tip：**
   *
   * 1.  `tip`: 基础库版本 1.1.0 开始支持，低版本需做[兼容处理](https://developers.weixin.qq.com/miniprogram/dev/framework/compatibility.html)
   * 2.  `tip`: 在没有调用`TaroH5.openBluetoothAdapter`的情况下调用小程序其它蓝牙模块相关API，API会返回错误，错误码为`10000`
   * 3.  `bug`: 在用户蓝牙开关未开启或者手机不支持蓝牙功能的情况下，调用`TaroH5.openBluetoothAdapter`会返回错误，错误码为`10001`，表示手机蓝牙功能不可用；此时小程序蓝牙模块已经初始化完成，可通过`TaroH5.onBluetoothAdapterStateChange`监听手机蓝牙状态的改变，也可以调用蓝牙模块的所有API。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.openBluetoothAdapter({
   *       success: function (res) {
   *         console.log(res)
   *       }
   *     })
   ...
   * @since 1.1.0
   *
   * 关闭蓝牙模块，使其进入未初始化状态。调用该方法将断开所有已建立的链接并释放系统资源。建议在使用小程序蓝牙流程后调用，与`TaroH5.openBluetoothAdapter`成对调用。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.closeBluetoothAdapter({
   *       success: function (res) {
   *         console.log(res)
   *       }
   *     })
   ...
   *
   *     ```javascript
   *     TaroH5.getBluetoothAdapterState({
   *       success: function (res) {
   *         console.log(res)
   *       }
   *     })
   ...
   *
   *     ```javascript
   *     TaroH5.onBluetoothAdapterStateChange(function(res) {
   ...
   *     TaroH5.startBluetoothDevicesDiscovery({
   *       services: ['FEE7'],
   *       success: function (res) {
   *         console.log(res)
   *       }
   ...
   *
   *     ```javascript
   *     TaroH5.stopBluetoothDevicesDiscovery({
   *       success: function (res) {
   *         console.log(res)
   *       }
   *     })
   ...
   * 3.  `tip`: 注意该接口获取到的设备列表为**小程序蓝牙模块生效期间所有搜索到的蓝牙设备**，若在蓝牙模块使用流程结束后未及时调用 TaroH5.closeBluetoothAdapter 释放资源，会存在调用该接口会返回之前的蓝牙使用流程中搜索到的蓝牙设备，可能设备已经不在用户身边，无法连接。
   ...
   * 1.  `tip`: Mac系统可能无法获取`advertisData`及`RSSI`，请使用真机调试
   * 2.  `tip`: 开发者工具和 Android 上获取到的`deviceId`为设备 MAC 地址，iOS 上则为设备 uuid。因此`deviceId`不能硬编码到代码中
   * 3.  `tip`: 若在onBluetoothDeviceFound回调了某个设备，则此设备会添加到 TaroH5.getBluetoothDevices 接口获取到的数组中
   *
   * **示例代码：**
   *
   *     ```javascript
   ...
   *       return hexArr.join('');
   *     }
   *     TaroH5.onBluetoothDeviceFound(function(devices) {
   ...
   *
   *     ```javascript
   *     TaroH5.getConnectedBluetoothDevices({
   *       success: function (res) {
   *         console.log(res)
   *       }
   *     })
   ...
   * 2.  `tip`: 蓝牙链接随时可能断开，建议监听 TaroH5.onBLEConnectionStateChange 回调事件，当蓝牙设备断开时按需执行重连操作
   * 3.  `tip`: 若对未连接的设备或已断开连接的设备调用数据读写操作的接口，会返回10006错误，详见错误码，建议进行重连操作
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.createBLEConnection({
   ...
   *
   *     ```javascript
   *     TaroH5.onBLEConnectionStateChange(function(res) {
   ...
   *
   *     ```javascript
   *     TaroH5.getBLEDeviceCharacteristics({
   *       // 这里的 deviceId 需要已经通过 createBLEConnection 与对应设备建立链接
   *       deviceId: deviceId,
   ...
   *     ```javascript
   *     // 必须在这里的回调才能获取
   *     TaroH5.onBLECharacteristicValueChange(function(characteristic) {
   *       console.log('characteristic value comed:', characteristic)
   *     })
   *
   *     TaroH5.readBLECharacteristicValue({
   ...
   *
   *     TaroH5.writeBLECharacteristicValue({
   ...
   * **Bug & Tip：**
   *
   * 1.  `tip`: 订阅操作成功后需要设备主动更新特征值的value，才会触发 TaroH5.onBLECharacteristicValueChange 回调。
   * 2.  `tip`: 安卓平台上，在调用notify成功后立即调用write接口，在部分机型上会发生 10008 系统错误
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.notifyBLECharacteristicValueChange({
   ...
   *       return hexArr.join('');
   *     }
   *     TaroH5.onBLECharacteristicValueChange(function(res) {
   ...
   *
   *     ```javascript
   *     TaroH5.onUserCaptureScreen(function(res) {
   *         console.log('用户截屏了')
   *     })
   *     ```
   ...
   *     TaroH5.startHCE({
   *       success: function(res) {
   *         TaroH5.onHCEMessage(function(res) {
   *           if (res.messageType === 1) {
   *             TaroH5.sendHCEMessage({data: buffer})
   *           }
   *         })
   *       }
   *     })
   ...
   *
   *     ```javascript
   *     TaroH5.onGetWifiList(function(res) {
   ...
   * @since 1.1.0
   *
   * 显示 loading 提示框, 需主动调用 [TaroH5.hideLoading](https://developers.weixin.qq.com/miniprogram/dev/api/api-react.html#wxhideloading) 才能关闭提示框
   ...
   * **Bug & Tip：**
   *
   * 1.  `bug`: `Android` `6.3.30`，TaroH5.showModal 的返回的 confirm 一直为 true；
   * 2.  `tip`: TaroH5.showActionSheet 点击取消或蒙层时，回调 `fail`, errMsg 为 "showActionSheet:fail cancel"；
   * 3.  `tip`: TaroH5.showLoading 和 TaroH5.showToast 同时只能显示一个，但 TaroH5.hideToast/TaroH5.hideLoading 也应当配对使用；
   * 4.  `tip`: `iOS` TaroH5.showModal 点击蒙层不会关闭模态弹窗，所以尽量避免使用“取消”分支中实现业务逻辑。
   *
   * **示例代码：**
   *
   *     ```javascript
   *     TaroH5.showActionSheet({
   *       itemList: ['A', 'B', 'C'],
   *       success: function(res) {
  ...
  /**
   * 保留当前页面，跳转到应用内的某个页面，使用`TaroH5.navigateBack`可以返回到原页面。
   *
   * **示例代码：**
   *
   *     ```javascript
   ...
   * **Tip：**
   *
   * 1.  `tip`: TaroH5.navigateTo 和 TaroH5.redirectTo 不允许跳转到 tabbar 页面，只能用 TaroH5.switchTab 跳转到 tabbar 页面
   *
   * **示例代码：**
   *
   *     ```javascript
   ...
   *
   *     ```javascript
   *     var animation = TaroH5.createAnimation({
   ...
   *         var query = TaroH5.createSelectorQuery()
   ...
   *     Component({
   *       queryMultipleNodes: function(){
   *         var query = TaroH5.createSelectorQuery().in(this)
   ...
   *     Page({
   *       getRect: function(){
   *         TaroH5.createSelectorQuery().select('#the-id').boundingClientRect(function(rect){
   *           rect.id      // 节点的ID
   ...
   *         TaroH5.createSelectorQuery().selectAll('.a-class').boundingClientRect(function(rects){
   ...
   *         TaroH5.createSelectorQuery().selectViewport().scrollOffset(function(res){
   *           res.id      // 节点的ID
   ...
   *     Page({
   *       getFields: function(){
   *         TaroH5.createSelectorQuery().select('#the-id').fields({
   *           dataset: true,
   *           size: true,
   *           scrollOffset: true,
   ...
   * **Bug & Tip：**
   *
   * 1.  `TaroH5.getExtConfig` 暂时无法通过 `TaroH5.canIUse` 判断是否兼容，开发者需要自行判断 `TaroH5.getExtConfig` 是否存在来兼容
   *
   * **示例代码：**
   *
   *     ```javascript
   *     if(TaroH5.getExtConfig) {
   *       TaroH5.getExtConfig({
   *         success: function (res) {
   ...
   * **Bug & Tip：**
   *
   * 1.  `TaroH5.getExtConfigSync` 暂时无法通过 `TaroH5.canIUse` 判断是否兼容，开发者需要自行判断 `TaroH5.getExtConfigSync` 是否存在来兼容
   *
   * **示例代码：**
   *
   *     ```javascript
   *     let extConfig = TaroH5.getExtConfigSync? TaroH5.getExtConfigSync(): {}
  ...
  /**
   * 调用接口TaroH5.login() 获取**临时登录凭证（code）**
   *
   * **示例代码：**
   *
   *     ```javascript
   ...
   *
   *     ```javascript
   *     // 可以通过 TaroH5.getSetting 先查询一下用户是否授权了 "scope.record" 这个 scope
   *     TaroH5.getSetting({
   *         success(res) {
   ...
   *                     scope: 'scope.record',
   *                     success() {
   *                         // 用户已经同意小程序使用录音功能，后续调用 TaroH5.startRecord 接口不会弹窗询问
   *                         TaroH5.startRecord()
   *                     }
   *                 })
   *             }
  ...
  /**
   * 获取用户信息，withCredentials 为 true 时需要先调用 [TaroH5.login](https://developers.weixin.qq.com/miniprogram/dev/api/api-login.html#wxloginobject) 接口。
   *
   ...
   * @since 1.2.0
   *
   * 获取用户过去三十天微信运动步数，需要先调用 [TaroH5.login](https://developers.weixin.qq.com/miniprogram/dev/api/api-login.html#wxloginobject) 接口。
   *
   ...
   *
   *     ```javascript
   *     TaroH5.navigateBackMiniProgram({
   *       extraData: {
   *         foo: 'bar'
   *       },
   *       success(res) {
   ...
   *
   *     ```javascript
   *     TaroH5.checkIsSupportSoterAuthentication({
   *         success(res) {
   ...
   *
   *     ```javascript
   *     TaroH5.startSoterAuthentication({
   ...
   *
   *     ```javascript
   *     TaroH5.checkIsSoterEnrolledInDevice({
   ...
   *
   *     ```javascript
   *     TaroH5.reportAnalytics('purchase', {
   *       price: 120,
   *       color: 'red'
   *     })
   *     ```
   ...
   *
   *     ```javascript
   *     const updateManager = TaroH5.getUpdateManager()
   *
   ...
   *
   *     ```javascript
   *     const worker = TaroH5.createWorker('workers/request/index.js') // 文件名指定 worker 的入口文件路径，绝对路径
   *
  ...
  class CanvasContext {
     ...
     *
     *     ```js
     *     const ctx = TaroH5.createCanvasContext('myCanvas')
     ...
     *
     * **Tip**: 绘图上下文需要由 `TaroH5.createCanvasContext(canvasId)` 来创建。
     *
     * **参数：**
     *
     *   参数       |  类型       |  说明                                                                                                                                       | 最低版本
